...
15. p35 2.2.4 章节第一小节：受压迫的种姓（Scheduled Castes，简称CS），简称错误，应为SCs或SC，参考自[Wiki](https://zh.wikipedia.org/wiki/%E8%A1%A8%E5%88%97%E7%A7%8D%E5%A7%93%E5%92%8C%E8%A1%A8%E5%88%97%E9%83%A8%E8%90%BD)；
16. p48 表4.2下方的描述内容`包括丈夫是否曾经对你有过殴打等行为，。`，疑似笔误多打了一个逗号`，`。紧接着下一句，`NHHS4调查中关于受访者回答的描述性统计见表4.3.`存在两个问题：一是`NHHS4`疑似为`NFHS4`的笔误，二是该句子以半角符号`.`结尾，明显存在标点符号混用现象；
17. p64，**生育影响家庭暴力的机制——母职惩罚**节标题下方的描述文字中，`因此本文在这里只计算了印度人口大邦（占全样本95%以上）的母职惩罚）。`，标点符号不规范，多打了右括弧`）`；
